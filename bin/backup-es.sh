...
CONTAINER="binoas_elastic_1"

# Get the name of the latest backup
LAST_SNAPSHOT=`sudo docker exec $CONTAINER curl -s 'http://localhost:9200/_snapshot/local_fs_backups/_all?pretty' |jq -r '.snapshots[-1] |.snapshot |@text'`
...
HTTP_STATUS=$(sudo docker exec $CONTAINER curl -s -o /dev/null -w "%{http_code}" -XPUT "http://localhost:9200/_snapshot/local_fs_backups/$NEW_SNAPSHOT?wait_for_completion=true")
...
if [[ "$HTTP_STATUS" -eq 200 || "$HTTP_STATUS" -eq 201 ]]; then
  ...
  sudo docker exec $CONTAINER curl -s -o /dev/null -XDELETE "http://localhost:9200/_snapshot/local_fs_backups/$LAST_SNAPSHOT"
else
